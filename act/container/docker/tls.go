...
// ssh:// shortcut). When an endpoint is given without tlsConf, DOCKER_TLS_VERIFY
// and DOCKER_CERT_PATH from the environment still apply as a fallback. TLS on an
// ssh:// endpoint is rejected because connhelper provides its own transport.
func Open(ctx context.Context, endpoint string, tlsConf *TLSConfig) (Endpoint, error) {
	...
	default:
		// With no PEM material, honour DOCKER_TLS_VERIFY/DOCKER_CERT_PATH from
		// the environment; FromEnv goes first so the explicit host still wins
		// (mirrors dialDockerDaemon, #1570).
		if tlsConf == nil {
			opts = append(opts, client.FromEnv)
		}
