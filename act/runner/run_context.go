...
// pluginCreateOpts builds the backend-options map passed to a plugin on Create,
// layering the label arg and job timeout over the plugin's configured options.
func (rc *RunContext) pluginCreateOpts(ctx context.Context, pluginOpts map[string]string) map[string]string {
	opts := make(map[string]string, len(pluginOpts)+2)
