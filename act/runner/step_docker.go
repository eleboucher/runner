...
func (sd *stepDocker) runUsesContainer() common.Executor {
	...
	return func(ctx context.Context) error {
		...
		stepContainer := sd.newStepContainer(ctx, docker.NewExecutionEnvironment(ep), image, cmd, entrypoint)
...
func (sd *stepDocker) newStepContainer(ctx context.Context, xe *docker.ExecutionEnvironment, image string, cmd, entrypoint []string) container.Container {
	rc := sd.RunContext
	step := sd.Step
